...
/*
    ...
    => VALUES 부본 대신 서브쿼리로 조회된 결과값을 통채로 INSERT 하는 방법
*/
...
-- 부서코드가 'D1'인 사번, 사원명, 부서코드, 사수사번, 입사일 정보조회
...
/*
    ...
    UPDATE 테이블명
        SET 컬럼 = 값, 
            컬럼 = 값, ...
    [WHERE 조건식] --> WHERE절을 생략했을 경우 테이블의 모든 행의 SET절에 데이터가 변경
    * 업데이트 시 제약조건을 잘 확인해야 함!
*/
--DEPT_TABLE 테이블에 DEPARTMENT 테이블을 복재
...
-- 사원(EMPLOYEE) 테이블을 EMP_TABLE 테이블로 복재(사번, 이름, 급여, 보너스 정보만)
...
-- 대북혼 사원의 급여를 500만원, BONUS 0.2로 변경
UPDATE EMP_TABLE
    SET SALARY = 5000000,
        BONUS = 0.2
    WHERE EMP_NAME = '대북혼';
    
-- 전체 사원의 기존 급여에 10프로 인상!
UPDATE EMP_TABLE
...
--아시아 지역에서 근무중인 사원들의 보너스 값을 0.3으로 변경
SELECT *
FROM LOCATION
