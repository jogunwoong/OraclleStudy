...
/*
    *시퀀스 생성하기*
    ...
    CREATE SEQUENCE 시퀀스명 
        [START WITH 숫자]  -->처음 발생시킬 시작값 지정[생략시 기본값은 1]
        [INCREMENT BY 숫자] --> 얼마만큼씩 증가시킬 것 인지에 대한 값 지정
        ...
        [NOCACHE] -->캐시 메모리 할당여부 [기본값: CACHE 20]
        [CACHE] --> 캐시메모리 미리 발생될 값들을 생성해서 저장해두는 공간
                    매번 호출할 때 마다 새로 번호를 생성하는 것이 아니라
...
FROM DUAL; -- NEXTVAL를 한번도 수행하지 않은 이상 CURRVAL는 사용할 수 없음!

SELECT SEQ_EMPNO.NEXTVAL
FROM DUAL; --생성후 처음 수행 시 시작 값 : 300
SELECT SEQ_EMPNO.CURRVAL
FROM DUAL;
SELECT SEQ_EMPNO.NEXTVAL
FROM DUAL;
...
--최대값이 310으로 설정되어 있어 그 이상의 갓이 조회되므로 오류 발생
/*
    * 시퀀스 구조 변경하기 *
