/*
    * 트리거 (TRIGGER)
     :지정한 테이블에 DML(INSERT, DELETE) 문에 의해 변경사항이 있을 때
        자동으로 매번 실행할 내용을 미리 정의해두는 것
        
        EX) 회원 탈퇴 시 기존 회원테이블에서 데이터삭제(DELETE)하고
        ...
        * 트리거의 종류 *
        ...
        CREATE [OR REPLACE] TRIGGER 트리거이름(트리거 기본 정보)
        BEFORE|AFTER INSERT|UPDATE|DELETE ON 테이블명(이벤트 관련 정보)       
        [FOR EACH ROW](생략가능]    매번 트리거를 실행할 것인지에 대한 옵션
