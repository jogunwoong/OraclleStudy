...
/*
    ...
    - 데이터 타입수정 : ALTER TABLE 테이블명 MODIFY 컬럼명 변결할 데이터 타입
    - 기본값 수정 : ALTER TABLE 테이블명 MODIFY 컬럼명 DEFALT 변결할 기본값
...
-- * 숫자타입으로 변경
ALTER TABLE DEPT_TABLE MODIFY DEPT_ID NUMBER; --> 형식오류 발생! (기존: 문자타입, 변경: 숫자타입)X
--DEPT_TABLE 테이블의 DEPT_TITLE 컬럼을 변경
---크기를 35바이트에서 10바이트로 변경
...
/*
    ...
    * 외래키 (FOREIGN KEY) : ALTER TABLE 테이블명 ADD FOREIGN KEY (컬럼명) REFRENCES 참조할테이블명![(참조할 컬럼명)]
...
--> 어딘가에 참조되고 있는 부모 테이블은 삭제가 되지 않음
-- 지우고자 할 경우, 1) 자식테이블을 먼저 삭제 후 부모테이블을 삭제
--                2) 부모테이블만 삭제 하는데, 제약조건까지 삭제
...
-- 2)제약조건명 변경 : RENAME CONSTRAINT 기존제약조건명 TO 바꿀제약제약조건명
ALTER TABLE DEPT_TABLE RENAME CONSTRAINT SYS_C008473 TO DT_DEPTID_NN;
...
------------------------------
-- * TRUNCATE : 테이블 초기화
-- => DROP 과는 다르게 테이블의 데이터만을 전부 삭제하여 테이블의 초기상태로 돌려주는것
